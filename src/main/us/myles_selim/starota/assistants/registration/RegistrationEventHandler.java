...
public class RegistrationEventHandler {
	...
	public void onServerCreate(GuildCreateEvent event) {
		if (!event.getGuild().getUsers().contains(
				RegistrationBot.REGISTRATION_CLIENT.getUserByID(StarotaConstants.STAROTA_ID))) {
		...
		if (!server.getUsers().contains(RegistrationBot.REGISTRATION_CLIENT.getOurUser()))
			return;
		IUser selimUser = RegistrationBot.REGISTRATION_CLIENT.fetchUser(StarotaConstants.SELIM_USER_ID);
		...
		if (!RegistrationBot.REGISTRATION_CLIENT.getOurUser().getPermissionsForGuild(server)
